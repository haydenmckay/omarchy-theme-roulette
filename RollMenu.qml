...
// currently typing.
//
// The hour/minute/interval steppers are `Stepper.qml`, a separate file --
// not an inline `component Stepper: Item {...}` block, which briefly lived
// right here and failed to parse at all (a QML syntax error that silently
// broke the whole file, and with it the right-click menu). See
// Stepper.qml's own header for why inline `component` is avoided in this
// codebase.
Item {
  id: root
